...
<div>
	 <h3 style="font-weight:bold;text-align:center;">Subsidiary Wise Standard Data</h3>
	 </div>
	...
	<table class="table table-bordered">
		...
		<tbody>
			<?php foreach ($values as $key): if(!empty($key['cadre'])){ ?>
				<tr>
					<td><?=$key['cadre']?></td>
					<td><?=$key['e1']?></td>
					<td><?=$key['e2']?></td>
			...
			echo'
			  ...
			  echo '<td>'.$total_e5.'</td>';echo '<td>'.$total_e6.'</td>';echo '<td>'.$total_e7.'</td>';
			  echo '<td>'.$total_e8.'</td>'; echo '<td><b>'.($result1).'</b></td></tr>';
			?>
